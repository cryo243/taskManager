(function() {
	...
	describe('user service', function() {
		...
		beforeEach(inject(function(_user_, _$httpBackend_, _$log_) {
			user = _user_;
		...
		describe('authenticate function', function() {
			...
			it('should return data', function() {
				var data = null;
				$httpBackend.when('POST', user.endpoint.host + user.endpoint.authenticate).respond(200, [
					{token: 'value'}
				]);
			...
			it('should log an error', function() {
				$httpBackend.when('POST', user.endpoint.host + user.endpoint.authenticate).respond(500);
